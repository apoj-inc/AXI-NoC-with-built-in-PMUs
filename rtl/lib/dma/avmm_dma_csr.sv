module avmm_dma_csr #(
    ...
    output logic [64:0]                dma_addr_o         [DMA_CHANNEL_COUNT],
    ...
    input  logic [DMA_TQ_ADDR_WIDTH:0] task_fifo_count_i  [DMA_CHANNEL_COUNT]
);
    ...
    typedef struct packed {
        ...
        logic [DMA_TQ_ADDR_WIDTH:0] task_fifo_count ;
    } dma_csr_struct_t;
    ...
    localparam TASK_COUNT      = 16'h0024;
    ...
    // Read collector
    logic [31:0] csr_rdata [1+DMA_CHANNEL_COUNT];
    
    always_comb begin
        translated_rdata = '0;
        for (int i = 0; i < 1+DMA_CHANNEL_COUNT; i++) begin
            translated_rdata |= csr_rdata[i];
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            endcase
            avmm_s_readdatavalid <= avmm_s_chipselect & avmm_s_read & !avmm_s_waitrequest;
        end
    end
    ...
    always_ff @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            case (translated_addr[DMA_STRUCT_ADDR_WIDTH-1:0])
                INFO_REG: csr_rdata[0] <= info_register;
                default : csr_rdata[0] <= '0;
            endcase
        end
    end
    ...
    generate
        ...
        for (i = 0; i < DMA_CHANNEL_COUNT; i++) begin : gen_structures
            ...
            always_ff @(posedge clk or negedge rst_n) begin
                if (!rst_n) begin
                    ...
                    if (struct_addr_enable) begin
                        case (translated_addr[DMA_STRUCT_ADDR_WIDTH-1:0])
                            CAP_NEXT_PTR    : csr_rdata[i+1] <= dma_csr_struct.cap_next_ptr    ;
                            DMA_ADDR_LO     : csr_rdata[i+1] <= dma_csr_struct.dma_addr[31:0]  ;
                            DMA_ADDR_HI     : csr_rdata[i+1] <= dma_csr_struct.dma_addr[63:32] ;
                            DMA_W_BYTES_REG : csr_rdata[i+1] <= dma_csr_struct.dma_word_bytes  ;
                            MAX_WR_LEN      : csr_rdata[i+1] <= dma_csr_struct.max_wr_len      ;
                            MAX_RD_LEN      : csr_rdata[i+1] <= dma_csr_struct.max_rd_len      ;
                            TQ_DEPTH        : csr_rdata[i+1] <= dma_csr_struct.tq_depth        ;
                            WDATA_CONUT     : csr_rdata[i+1] <= dma_csr_struct.wdata_fifo_count;
                            RDATA_FREE      : csr_rdata[i+1] <= dma_csr_struct.rdata_fifo_free ;
                            TASK_COUNT      : csr_rdata[i+1] <= dma_csr_struct.task_fifo_count ;
                            default         : csr_rdata[i+1] <= '0                             ;
                        endcase
                    end
                    else begin
                        csr_rdata[i+1] <= '0;
            ...
            assign dma_csr_struct.cap_next_ptr   = (i == DMA_CHANNEL_COUNT) ? '0 : ((i+2) << DMA_STRUCT_ADDR_WIDTH);
            ...
            always_ff @(posedge clk or negedge rst_n) begin
                if (!rst_n) begin
                    ...
                    dma_csr_struct.task_fifo_count  <= '0                              ;
                end
                else begin
                    ...
                    if (struct_addr_enable && avmm_s_write) begin
                        case (translated_addr[DMA_STRUCT_ADDR_WIDTH-1:0])
                            ...
                            TASK_COUNT      : /*dma_csr_struct.task_fifo_count  <= translated_wdata*/ ; // Read-only
                            default         :                                                    ;
                        endcase
                    end
                    ...
                    dma_csr_struct.task_fifo_count  <= task_fifo_count_i [i];
                end
            end
        end
    endgenerate
