        ...
        for (i = 0; i < ROUTERS_COUNT; i++) begin : router_iteration
            ...
            for (generatic = 0; generatic < GENERATICS_COUNT; generatic++) begin : interconnection_assignment
                for (current_virtual_channel = 0; current_virtual_channel < VIRTUAL_CHANNEL_NUMBER; current_virtual_channel++) begin : assigning_generatics_channels
                    ...
                    // Counter clockwise
                    `AXIS_INTERFACE2INTERFACE(router_if[i][(generatic+GENERATICS_COUNT+1)*VIRTUAL_CHANNEL_NUMBER+current_virtual_channel]  , router_in[(i+ROUTERS_COUNT-GENERATICS[generatic])%ROUTERS_COUNT][(generatic+GENERATICS_COUNT+1)*VIRTUAL_CHANNEL_NUMBER+current_virtual_channel])
                end
            end
